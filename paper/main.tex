...
\IEEEauthorblockA{Independent Study}
}
...
\texttt{parallel\_multiple}, and \texttt{irrelevance}. These are subset results, not BFCL
leaderboard scores. The main model is LFM2-8B-A1B-UD-Q2\_K\_XL, a 2-bit quantization with
...
an unbiased model-selection result. In a follow-up evaluation, all models and configurations
should be frozen before scoring held-out examples. Each run should report the exact model
...
\section{Proposed Controlled Study}

The current artifact makes a broader study possible, but that study has not been run at the
time of writing. We propose the following preregistered-style comparison:
