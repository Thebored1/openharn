...
semantic knowledge or guarantee correct dependent multi-step planning. Planning can improve in
some cases when the presentation and generation path match the model, but residual decomposition
errors remain. We provide a failure taxonomy and clearly separate completed measurements from
future evaluation.
\end{abstract}
...
rather than a new trained model~\cite{openharn2026}. The harness is designed for small models and exposes the
...
category at Q8 and Q4. It tests whether native server-side inference controls---
\texttt{tool\_choice=required} and \texttt{enable\_thinking:false} through the chat-template
options---can recover quantization degradation without changing weights.
...
degradation on a different model family, MiniCPM-V. The custom native-template plus foreign
array-grammar path was tested but underperformed; the reported intervention is the native
server path with \texttt{tool\_choice=required} and thinking disabled. Table~\ref{tab:quant}
reports the separate experiment. The Q4 model's score rises
