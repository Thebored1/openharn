...
\title{Inference-Time Recovery of Tool-Use Capability in Ultra Quantised Large Language Models}
...
A prompt-tools configuration initially scores 29.5\%, while the later per-request policy
reaches 63.0\% on a full 200-example run; a subsequent decomposition fix reaches 64.5\%. On a
...
The current experiments answer RQ1 and partially answer RQ2 and RQ4. RQ3 is implemented in
the artifact but requires the proposed controlled comparison before quantitative conclusions
can be made.
...
Q4 required + no-think & 72.5\% & 72.5/72.5/72.5 & ${\sim}4\times$ faster\\
\bottomrule
\end{tabular}
\end{table}
...
latency, especially on local hardware. Disabling thinking can improve throughput, but it may
change planning quality; the speed/accuracy tradeoff must be reported rather than assumed.
...
BFCL v4 datasets and evaluation code are from the Gorilla project. We thank the llama.cpp
community for the OpenAI-compatible inference interface used in the experiments. The reported
runs were conducted on consumer CPU-oriented hardware.
