...
| `OPENHARN_PLAN_ALWAYS` | like `PLAN_FIRST`, but also runs the enumeration step for **thinking** models — *after* their native think block, before the grammar. (`PLAN_FIRST` alone is skipped when the template opens a think tag, because the model already reasons.) | a *reasoning* model that thinks but still under-calls on composition (BFCL: MiniCPM-Q4 `parallel` 35→~49% mean, but adds a 2nd free-generation phase so run-to-run variance rises). No effect on non-thinking models beyond `PLAN_FIRST` |
